CREATE OR REPLACE TABLE KSFDA.SHRINK_TACTICAL.PAPER_SHRINK_DETAILED6 AS
(
...
WITH CTE_RNK AS(
	...
	FROM KSFDA.SHRINK_TACTICAL.IPS_DETAIL_FY24 ips 
	JOIN KSFDA.SHRINK_TACTICAL.MR_DETAIL_FY24 mr
...
CTE_MR AS(
	SELECT * exclude(RNK) FROM KSFDA.SHRINK_TACTICAL.MR_DETAIL_FY24
	PIVOT(SUM(LINE_VALUE) FOR LINE_ITEM IN ('MANIFEST', 'SALES', 'SHRINK', 'ITC', 'RLO', 'PO'))
	),
CTE_IPS AS(
	SELECT * exclude(RNK) FROM KSFDA.SHRINK_TACTICAL.IPS_DETAIL_FY24
	PIVOT(SUM(LINE_VALUE) FOR LINE_ITEM IN ('MANIFEST', 'SALES', 'SHRINK', 'ITC', 'RLO', 'PO'))
	)
